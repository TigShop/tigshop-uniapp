                    ...
                    <view class="coupon-detail">
                        <view class="coupon-detail-item">
                            ...
                            <view class="coupon-detail-item-right">
                                ...
                                <view class="item-right-time" v-if="pageType == 1"> {{ item?.use_start_date }} - {{ item?.use_end_date }} </view>
                                <view class="item-right-time" v-else> 截止{{ item?.end_date }} </view>
                                <view class="tmcscoupon-item-btn">
                                    <tigButton v-if="pageType == 1 && item.is_receive" :plain="true"  @click="handleReceiveCoupon(item.coupon_id)"> 立即领取 </tigButton>
                                    <tigButton v-if="pageType == 1 && !item.is_receive" :plain="true"  @click="handleReceiveCoupon(item.coupon_id)"> 已使用 </tigButton>
                                    
                                    <tigButton v-if="pageType == 2 && (item.status === 1 || item.status === 2)" :plain="true" plainMainColor  @click="handleReceiveCoupon(item.coupon_id)"> 立即使用 </tigButton>
                                    <tigButton v-if="pageType == 2 && item.status === 4" :plain="true"> 已使用 </tigButton>
                                    <tigButton v-if="pageType == 2 && item.status === 5" :plain="true"> 已过期 </tigButton>
                            </view>
                            </view>
                        </view>
                    </view>
